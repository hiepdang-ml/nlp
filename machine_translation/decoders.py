...
class GRUDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.gru = nn.GRU(
            input_size=embedding_dim + n_hiddens * 2, 
            hidden_size=n_hiddens, 
            num_layers=n_layers, 
            dropout=dropout, 
            bidirectional=True
        )
        self.dense = nn.Linear(in_features=self.n_hiddens * 2, out_features=vocabulary_size)
    ...
    def forward(
        ...
        context = encoder_output[-1]    # shape (batch_size, n_hiddens * 2)
        context = context.repeat(n_steps, 1, 1) # broadcast to (n_steps, batch_size, n_hiddens * 2)
        assert context.shape == (n_steps, batch_size, self.n_hiddens * 2)
        # The context variable is concatenated with the decoder input at all the time steps
        embeddings_and_context = torch.cat(tensors=(embeddings, context), dim=2) # shape (n_steps, batch_size, embedding_dim + n_hiddens * 2)
        output: torch.Tensor    # shape (n_steps, batch_size, n_hiddens * 2)
        hidden_state: torch.Tensor  # shape (n_layers * 2, batch_size, n_hiddens)
        ...
        assert hidden_state.shape == (self.n_layers * 2, batch_size, self.n_hiddens)
